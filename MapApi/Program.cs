...
app.MapGet("/api/v1/admin/stats", async (AppDb db, IDevicePresenceService presence) =>
{
    ...
    var displayOnlineVisitors = presence.DisplayOnlineVisitors;
    ...
    return Results.Ok(new
    {
        ...
        DisplayOnlineVisitors = displayOnlineVisitors,
...
// ============================================================
// GET /api/v1/admin/visitor-poi-regions — POI gần visitor theo GPS mới nhất
// ============================================================
app.MapGet("/api/v1/admin/visitor-poi-regions", async (
    AppDb db, IDevicePresenceService presence, CancellationToken ct) =>
{
    const double betweenPoiDeltaMeters = 30.0;

    var devices = (await db.GuestDevices.AsNoTracking()
        .Where(d => d.LastLatitude.HasValue && d.LastLongitude.HasValue)
        .OrderByDescending(d => d.LastActiveAt)
        .Select(d => new
        {
            d.DeviceId,
            d.Platform,
            d.AppVersion,
            Latitude = d.LastLatitude!.Value,
            Longitude = d.LastLongitude!.Value,
            d.LastActiveAt
        })
        .ToListAsync(ct))
        .Where(d => presence.IsOnline(d.DeviceId))
        .ToList();
    ...
    {
        var nearestPois = pois
            .Select(p => new
            {
                Poi = p,
                DistanceMeters = GeoDistanceMeters(d.Latitude, d.Longitude, p.Latitude, p.Longitude)
            })
            .OrderBy(x => x.DistanceMeters)
            .Take(2)
            .ToList();

        var first = nearestPois.ElementAtOrDefault(0);
        var second = nearestPois.ElementAtOrDefault(1);

        // Visitor is between two POIs when the top two distances are nearly equal.
        var relation = second is not null &&
                       Math.Abs(second.DistanceMeters - first!.DistanceMeters) <= betweenPoiDeltaMeters
            ? "BETWEEN_2_POIS"
            : "NEAR_SINGLE_POI";
        ...
        {
            DeviceId = d.DeviceId,
            VisitorName = string.IsNullOrWhiteSpace(d.Platform)
                ? $"Visitor {d.DeviceId[..Math.Min(8, d.DeviceId.Length)]}"
                : $"{d.Platform} visitor",
            Latitude = d.Latitude,
            ...
            d.AppVersion,
            d.LastActiveAt,
            NearestPoiId = first?.Poi.Id,
            NearestPoiName = first?.Poi.Name,
            SecondPoiId = second?.Poi.Id,
            SecondPoiName = second?.Poi.Name,
            DistanceToNearest = first is null ? (double?)null : Math.Round(first.DistanceMeters, 1),
            DistanceToSecond = second is null ? (double?)null : Math.Round(second.DistanceMeters, 1),
            Relation = first is null ? "NO_POI_AVAILABLE" : relation
        };
    }).ToList();
    ...
    return Results.Ok(new
    {
        ...
        ActiveVisitorCount = items.Count,
        BetweenThresholdMeters = betweenPoiDeltaMeters,
        Items = items
    });
});

// ============================================================
// POST /api/v1/audio-sessions/start — App báo bắt đầu nghe audio POI
// ============================================================
app.MapPost("/api/v1/audio-sessions/start", (
    AudioSessionRequest req, IAudioQueueService queue) =>
{
    if (string.IsNullOrWhiteSpace(req.DeviceId))
        return Results.BadRequest(new { error = "DeviceId là bắt buộc" });
    if (req.PoiId <= 0)
        return Results.BadRequest(new { error = "PoiId là bắt buộc" });

    var sessionId = string.IsNullOrWhiteSpace(req.SessionId) ? Guid.NewGuid().ToString("N") : req.SessionId.Trim();
    var session = queue.Start(req.DeviceId.Trim(), req.PoiId, req.VisitorLabel, sessionId);
    return Results.Ok(new
    {
        ok = true,
        session.SessionId,
...
// ============================================================
// POST /api/v1/audio-sessions/end — App báo dừng/phát xong audio POI
// ============================================================
app.MapPost("/api/v1/audio-sessions/end", (
    AudioSessionRequest req, IAudioQueueService queue) =>
{
    if (string.IsNullOrWhiteSpace(req.DeviceId))
        return Results.BadRequest(new { error = "DeviceId là bắt buộc" });
    ...
    if (string.IsNullOrWhiteSpace(req.SessionId))
        return Results.BadRequest(new { error = "SessionId là bắt buộc" });

    var ended = queue.End(req.DeviceId.Trim(), req.PoiId, req.SessionId.Trim());
    return Results.Ok(new { ok = true, ended });
});

// ============================================================
// GET /api/v1/admin/audio-queues — Queue visitor đang nghe audio theo POI
// ============================================================
app.MapGet("/api/v1/admin/audio-queues", async (
    AppDb db, IAudioQueueService queue, CancellationToken ct) =>
{
    var sessions = queue.GetActiveSessions();
    var poiIds = sessions.Select(x => x.PoiId).Distinct().ToList();
    var poiNames = await db.Pois.AsNoTracking()
    ...
    var groups = sessions
        ...
        .Select(g => new
        {
            ...
            ActiveListeners = g.Count(),
            Queue = g.OrderBy(x => x.StartedAt).Select((x, index) => new
            {
                QueueNumber = index + 1,
                x.SessionId,
                x.DeviceId,
                VisitorLabel = x.VisitorLabel ?? $"Visitor {x.DeviceId[..Math.Min(8, x.DeviceId.Length)]}",
                x.StartedAt,
                x.LastSeenAt,
                x.Status
            }).ToList()
    ...
    {
        GeneratedAt = DateTime.UtcNow,
        TotalActiveListeners = sessions.Count,
        PoisWithActiveAudio = groups.Count,
        Groups = groups
    });
});
...
public sealed record AudioSessionRequest(string DeviceId, int PoiId, string? VisitorLabel, string? SessionId);
