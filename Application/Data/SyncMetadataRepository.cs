...
ON CONFLICT(Key) DO UPDATE SET Value=excluded.Value;";
    ...
    public async Task<DateTime?> GetLastSyncUtcAsync(string scope, CancellationToken ct = default)
    {
        await InitAsync();
        await using var conn = new SqliteConnection(Constants.ConnectionString);
        ...
        return DateTime.TryParse((string)value, out var parsed) ? parsed : null;
    }
}
